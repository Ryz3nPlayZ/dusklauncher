...
- Vanilla and Fabric differ only in the version JSON + classpath: Fabric's `https://meta.fabricmc.net/v2/versions/loader/<game_version>/<loader_version>/profile/json` is a partial piston profile (`inheritsFrom` semantics) whose `mainClass` is `net.fabricmc.loader.impl.launch.knot.KnotClient`. It ships `arguments` + its own libraries but **no** `javaVersion`/`downloads.client`/`assetIndex` — installing Fabric = fetch that profile and **merge it with the vanilla JSON**. Two invariants: (1) argument arrays **concatenate** (vanilla's + fabric's — fabric ships `"game": []`, and substituting wholesale drops `-cp ${classpath}`); (2) Fabric libraries are maven-dialect (`name` coordinates + top-level `url` repo base, checksums optional) with no `downloads.artifact` — `Library::resolve_artifact` normalizes both dialects for downloads and the classpath; skipping maven-dialect libs is the `ClassNotFoundException: KnotClient` failure.
