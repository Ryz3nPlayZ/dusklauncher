...
How custom GUI works in FasterClient (MC **1.21.11**, Fabric API 0.141.2,
...
| In-game settings screen, opened by Right Shift keybind | `gui/DuskSettingsScreen.java`, `FasterClient.java:48-57` |
...
| 6 registered modules (keystrokes, CPS, FPS, toggle-sprint, armor, combo) — state-only stubs | `FasterClient.java:37-43` |
| Texture upload precedent (NativeImage → DynamicTexture under `fasterclient:`) | `cosmetics/CapeTexture.java` |
...
HudLayerRegistrationCallback.EVENT.register(attach ->
    attach.attachLayerAfter(IdentifiedLayer.MISC_OVERLAYS,
        Identifier.fromNamespaceAndPath("fasterclient", "modules"),
        (guiGraphics, delta) -> FasterClient.modules().renderAll(guiGraphics, delta)));
```
...
font provider JSON under `assets/fasterclient/font/` — later.
...
1. Ship PNGs under `src/main/resources/assets/fasterclient/textures/gui/`,
   reference as `Identifier.fromNamespaceAndPath("fasterclient", "gui/panel")`.
...
3. For runtime files (the launcher wallpaper bridge below), upload a
   ...
   from `CapeTexture`, which already handles `fasterclient:` ids and
