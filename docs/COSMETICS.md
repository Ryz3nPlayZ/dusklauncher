...
```
client-mod/src/main/resources/assets/fasterclient/cosmetics/
...
- `client-mod` `gradle build` runs `installToLauncher`, which copies `remapJar` to `launcher/src-tauri/resources/fasterclient.jar`; `tauri.conf.json` bundles `resources/`, so the jar ships inside the app.
...
- `Cosmetics.tsx` CAPES tab: grid of `CapeSwatch` tiles (the cape's outer face on a 2D canvas, animated strips cycle at 100 ms), a NONE tile, the account's current skin in `PlayerRender` wearing the picked cape (skinview3d `loadCape` with `backEquipment: 'cape'`, strips split per frame client-side because skinview3d only takes single 64×32-scaled images) and `loadEars` for capes with ears. Green ring = worn, accent ring = previewed; APPLY CAPE / REMOVE CAPE → `set_loadout`. The browser preview (`npm run dev` outside Tauri) shows the real catalog: `vite.config.ts` serves `client-mod/src/main/resources/assets/fasterclient/cosmetics/` at `/__cosmetics`, and the stubs in `api.ts` fetch `registry.json` and the PNGs from there.
...
- **Store + inventory** (`Store.tsx`, route `store`): what the user *owns* lives in `<data>/inventory.json` (`{"owned": [ids]}`); `load_inventory` always unions in whatever the loadout wears, so a loadout written before the file existed is still "owned". Commands: `get_inventory()`, `claim_cosmetic(id)` (rejects ids not in the bundled catalog; everything is free until phase 5, when Tebex entitlements replace this file), `export_cosmetic_texture(kind, id)` (save dialog → the cape/ears/accessory PNG out of the jar, for the SAVE PNG FOR MINECRAFTCAPES button — the only way a Dusk cape reaches MinecraftCapes/Cosmetica viewers, see §3.1). The store previews the picked item on the account skin over the current look, GET claims, EQUIP/UNEQUIP writes the loadout slot directly. The wardrobe (`Cosmetics.tsx`) lists **owned** items only and ends every grid with a GET MORE tile that opens the store.
