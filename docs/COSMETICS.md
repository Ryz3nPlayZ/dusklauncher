...
- `client-mod` builds once per game line (`gradle build -Pmc=1.21.11` and `-Pmc=26.2`); each build's `installToLauncher` copies `remapJar` to `launcher/src-tauri/resources/duskclient-<mc>.jar`. `tauri.conf.json` bundles `resources/`, so both jars ship inside the app and `cosmetics::client_mod_jar_for` picks the one for the profile's version (1.21.x → `duskclient-1.21.11.jar`, 26.2 → `duskclient-26.2.jar`; any other version launches without the mod).
