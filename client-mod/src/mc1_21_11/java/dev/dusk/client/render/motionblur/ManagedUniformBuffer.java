...
/**
 * A UBO we own, swapped in over the placeholder the post-chain loader
 * allocates. Reload the resource pack and the chain is rebuilt, so the
 * buffer is keyed on its owning chain and dropped when that changes.
 */
public final class ManagedUniformBuffer {
    ...
    /** Several buffers in rotation, so a frame never writes the one still being read. */
