 ...
 * read every frame.
 *
 * 1.21–1.21.1 flavour: DynamicTexture takes just the NativeImage (no debug label);
 * NativeImage pixels are get/setPixelRGBA.
 */
public final class CapeTexture {
    ...
    private CapeTexture(ResourceLocation[] frames, NativeImage[] images, int frameMs) {
        this.frames = frames;
        this.images = images;
    ...
    public static CapeTexture prepareCape(String key, byte[] png, int frameMs) {
        ...
        try {
                ...
                for (int f = 0; f < total; f++) {
                    ...
                    for (int x = 0; x < w; x++) {
                        for (int y = 0; y < frameH; y++) {
                            frame.setPixelRGBA(x, y, src.getPixelRGBA(x, y + f * frameH));
                        }
                    }
                    out.add(frame);
                }
                ResourceLocation[] ids = new ResourceLocation[total];
            ...
            for (int x = 0; x < w; x++) {
                for (int y = 0; y < h; y++) {
                    padded.setPixelRGBA(x, y, src.getPixelRGBA(x, y));
                }
            }
            return new CapeTexture(new ResourceLocation[] {id("capes/" + key)}, new NativeImage[] {padded}, frameMs);
        } finally {
            src.close();
        }
    }
    ...
    public static CapeTexture prepareEars(String key, byte[] png) {
        try {
            ...
            return new CapeTexture(new ResourceLocation[] {id("ears/" + key)}, new NativeImage[] {img}, FRAME_MS);
        } catch (IOException e) {
            return null;
        }
    }
    ...
    public static CapeTexture prepareFrames(String key, byte[] png, int frames, int frameMs) {
        ...
        if (frames <= 1 || src.getHeight() % frames != 0) {
            return new CapeTexture(new ResourceLocation[] {id("accessories/" + key)}, new NativeImage[] {src}, frameMs);
        }
        try {
            int w = src.getWidth();
            int fh = src.getHeight() / frames;
            NativeImage[] out = new NativeImage[frames];
            ResourceLocation[] ids = new ResourceLocation[frames];
            for (int f = 0; f < frames; f++) {
                NativeImage frame = new NativeImage(w, fh, true);
                for (int x = 0; x < w; x++) {
                    for (int y = 0; y < fh; y++) {
                        frame.setPixelRGBA(x, y, src.getPixelRGBA(x, y + f * fh));
                    }
                }
                out[f] = frame;
    ...
    public void register() {
        ...
        for (int i = 0; i < frames.length; i++) {
            final ResourceLocation id = frames[i];
            tm.register(id, new DynamicTexture(images[i]));
    ...
    public void release() {
        ...
        if (registered) {
            TextureManager tm = Minecraft.getInstance().getTextureManager();
            for (ResourceLocation id : frames) tm.release(id);
        } else {
    ...
    private static ResourceLocation id(String path) {
        return ResourceLocation.fromNamespaceAndPath("duskclient", path);
    }
}
