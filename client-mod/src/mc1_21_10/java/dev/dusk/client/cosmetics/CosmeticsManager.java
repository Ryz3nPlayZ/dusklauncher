...
public final class CosmeticsManager {
    ...
    public static PlayerSkin skinFor(UUID uuid, @Nullable String name, PlayerSkin original) {
        ...
        if (c.hasCape()) {
            ...
            PlayerSkin out = new PlayerSkin(original.body(), c.capeAsset(), c.capeAsset(), original.model(), original.secure());
            e.lastIn = original;
            e.lastOut = out;
            return out;
        }
        if (original.cape() != null && DuskConfig.get().cosmetics.hideOfficialCapes) {
            if (e.lastIn == original && e.lastOut != null) return e.lastOut;
            PlayerSkin out = new PlayerSkin(original.body(), null, original.elytra(), original.model(), original.secure());
            e.lastIn = original;
            e.lastOut = out;
            return out;
        }
